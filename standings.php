...
class League
{
    ...
    public $winPct;
    public $matchPts;
    public function League($id, $name)
    {
        $this->id = $id;
        $this->name = $name;
    }
    ...
    public function teamCmp($a, $b)
    {
        //echo "In teamCmp, a's gamesArray is " . sizeof($a->gamesArray) . "<br/>";
    if ($c = $this->winPct($a, $b)) {
        return $c;
    } elseif ($c = $this->matchPct($a, $b)) {
        return $c;
    } elseif ($c = $this->head2Head($a, $b)) {
        return $c;
    } elseif ($c = $this->ptsAllowedOpponent($a, $b)) {
        return $c;
    } elseif ($c = $this->ptsAllowed($a, $b)) {
        return $c;
    } elseif ($c = $this->ptsDifferential($a, $b)) {
        return $c;
    } else {
        $a->addTie($b->id);
        $b->addTie($a->id);
    ...
    public function ptsAllowedOpponent($a, $b)
    {
    ...
    public function ptsAllowed($a, $b)
    {
    ...
    public function ptsDifferential($a, $b)
    {
        $ap = $a->getTotalPoints();
        $bp = $b->getTotalPoints();
        if ($ap == $bp) {
    ...
    if (!usort($this->teamArray, array(&$this, 'teamCmp'))) {
        die('Hey, usort failed!');
    }
...
class Team
{
    ...
    {
        $won = 0;
        if (sizeof($this->gamesArray) == 0) {
            return 0;
        }
        foreach ($this->gamesArray as $game) {
            if ($game->usmp > $game->themmp) {
                ++$won;
            }
        }
    ...
    { // Create method of not adding BYE/Cancelled games
...
EOF;
    ...
    public function getTotalMatchPoints()
    {
        $pts = 0;
        if (sizeof($this->gamesArray) == 0) {
            return 0;
    ...
    public function getTotalWinPct()
    {
        $gamesWon = 0;
        $gamesPlayed = 0;
        if (sizeof($this->gamesArray) == 0) {
    ...
    public function getOpponentWinPct($id)
    {
        $wins = 0;
        $gamesPlayed = 0;
        if (sizeof($this->gamesArray) == 0) {
    ...
    public function getPointsAllowedOpponent($id)
    {
        $ptsAllowed = 0;
        if (sizeof($this->gamesArray == 0)) {
            return 0;
    ...
    public function getTotalPointsAllowed()
    {
        $ptsAllowed = 0;
        if (sizeof($this->gamesArray) == 0) {
            return 0;
    ...
    public function getTotalMatchPct()
    {
        $gamesPlayed = 0;
        $matchPtsObtained = 0;
        if (sizeof($this->gamesArray) == 0) {
    ...
    public function getPossMatchPoints()
    {
        $gamesPlayed = 0;
        if (sizeof($this->gamesArray) == 0) {
            return 0;
...
class Game
{
    ...
    public function Game($us1, $us2, $us3, $them1, $them2, $them3, $usmp, $themmp, $opponent)
    {
        $this->us1 = $us1;
        $this->us2 = $us2;
        $this->us3 = $us3;
    ...
    public function getOppPoints()
    {
        return $this->them1 + $this->them2 + $this->them3;
    }

    public function getUsMatchPoints()
    {
        return $this->usmp;
    }
